...
// ... (Diğer key'ler aynı kalıyor) ...
const SOCIAL_MEDIA_KEY = 'kyrosil_socialMedia'; const SOCIAL_USER_KEY = 'kyrosil_socialUser'; const TK_MILES_KEY = 'kyrosil_tkMiles'; const MAVI_GSM_KEY = 'kyrosil_maviGsm'; const CARREFOURSA_INFO_KEY = 'kyrosil_carrefoursaInfo'; const SWISSAIR_NO_KEY = 'kyrosil_swissairNo'; const CARREFOUR_EU_KEY = 'kyrosil_carrefourEu'; const TIKTAK_GSM_KEY = 'kyrosil_tiktakGsm'; const TRENDYOL_EMAIL_KEY = 'kyrosil_trendyolEmail';


// Google Gemini API Ayarları
const GEMINI_API_KEY = 'AIzaSyDiMIy8gM65-DWVlneXq4oKW4lCqwK0nK4'; // !!! YENİ ANAHTARINI BURAYA YAPIŞTIR !!!
const API_URL = "https://generativelanguage.googleapis.com/v1beta/models/gemini-2.0-flash:generateContent?key=" + GEMINI_API_KEY; // 2.0 Flash ile devam ediyoruz

// === YENİ: Konuşma Geçmişi ===
let chatHistory = []; // { role: 'user'/'model', parts: [{ text: '...' }] } formatında tutacağız
const MAX_HISTORY_LENGTH = 6; // Son kaç mesajı hatırlasın (çift sayı olması iyi olur)
// =============================
...
// Mesajı sohbet kutusuna ekleyen ve geçmişe kaydeden fonksiyon (GÜNCELLENDİ)
function addMessage(text, role) {
    ...
    // Geçmiş çok uzadıysa en eskileri sil
    if (chatHistory.length > MAX_HISTORY_LENGTH) {
        chatHistory = chatHistory.slice(chatHistory.length - MAX_HISTORY_LENGTH);
    }
    
    // Konsola geçmişi yazdırabiliriz (debug için)
    // console.log("Chat History:", JSON.stringify(chatHistory, null, 2));

    scrollToBottom();
}

// Bot mesajı ekleme ve gecikme fonksiyonu (Artık addMessage kullanıyor)
function addBotMessage(text, delay = 600) {
    setTimeout(() => {
        addMessage(text, 'model'); // Rolü 'model' olarak gönderiyoruz
    }, delay);
}
...
function startConversation() {
    // İlk mesajları eklemeden önce geçmişi temizle (sayfa yenilendiğinde)
    chatHistory = []; 
    // Başlangıç mesajını ekleyelim (ama geçmişe değil, sadece görüntüye)
    // Not: İlk mesajı geçmişe eklememek, API'ye boşuna gitmemesini sağlar.
    // Eğer ilk mesajın da AI tarafından bilinmesi isteniyorsa, bu mesajı da addMessage ile eklemeliyiz.
    addMessageToChatBox("Avrupa'nın sponsor markalarla ilk ve tek tam entegre yapay zeka botu KyrosilRewards'a hoş geldin! ✨", 'bot-message');

    if (!currentUserName) { addBotMessage("Seninle tanışmak istiyorum, adın nedir?", 100); conversationState = 'awaiting_name'; }
...
// Gemini API Fonksiyonu (GÜNCELLENDİ - Geçmişi kullanıyor)
async function getGeminiResponse() { // Artık prompt parametresi almıyor, chatHistory'den alacak
    
    // Son mesajın kullanıcıdan geldiğinden emin olalım (teorik olarak hep öyle olmalı)
    if (chatHistory.length === 0 || chatHistory[chatHistory.length - 1].role !== 'user') {
        console.error("API'ye gönderecek kullanıcı mesajı bulunamadı.");
        return; 
    }

    addBotMessage("...", 100); // Thinking indicator

    // API'ye gönderilecek veri yapısı (artık tüm geçmişi içeriyor)
    const payload = {
        contents: chatHistory // Tüm konuşma geçmişini gönderiyoruz
        // generationConfig ve safetySettings eklenebilir
    };
    ...
    try {
        ...
        // Cevap yapısını kontrol et
        if (data.candidates && data.candidates.length > 0 && data.candidates[0].content && data.candidates[0].content.parts && data.candidates[0].content.parts.length > 0) {
            botReply = data.candidates[0].content.parts[0].text;
        } else if (data.promptFeedback && data.promptFeedback.blockReason){
             botReply = `İçerik güvenlik nedeniyle engellendi: ${data.promptFeedback.blockReason}`; console.error("API İçerik Engeli:", data.promptFeedback);
        ...
        if (thinkingMessage && thinkingMessage.textContent === "...") {
             ...
             // API cevabını geçmişe ekle ('...' yerine)
             ...
             // Eğer '...' bulunamazsa yeni mesaj olarak ekle (hem ekrana hem geçmişe)
...
// Kullanıcı mesaj gönderme fonksiyonu (GÜNCELLENDİ - addMessage kullanıyor, API çağrısı parametresiz)
function sendMessage() {
    const messageText = userInput.value.trim();
    if (messageText !== "") {
        // Kullanıcı mesajını ekle (hem ekrana hem geçmişe)
        ...
        if (conversationState !== 'idle') {
            ...
            if (messageText.startsWith('/')) {
                // Komut işleme (switch case'ler aynı)
                const command = messageText.substring(1).toLowerCase().trim();
                let commandHandled = true; // Komut işlenirse API'ye gitmesin
                switch (command) {
                    ...
                    default: 
                        addBotMessage(`Bilinmeyen komut: "${command}". Yardım için /help veya /sponsor-kayit yazabilirsin.`);
                        commandHandled = false; // Bilinmeyen komut API'ye gidebilir mi? Şimdilik gitmesin.
                }
                 // Eğer bilinen bir komutsa, API'ye gitme
                 if (commandHandled) return; 
            }
            
            // Komut değilse normal sohbet (API çağrısı)
            const lowerCaseInput = messageText.toLowerCase();
            let greetingHandled = true; // Selamlaşma işlenirse API'ye gitmesin
            ...
            else { greetingHandled = false; } // Selamlaşma değil

            // Eğer selamlaşma değilse ve API anahtarı geçerliyse API'yi çağır
            if (!greetingHandled) {
                if (typeof GEMINI_API_KEY !== 'undefined' && GEMINI_API_KEY && GEMINI_API_KEY !== 'SENIN_API_ANAHTARIN_BURAYA') {
                     getGeminiResponse(); // prompt parametresini kaldırdık, history'den alacak
                } else {
                     addBotMessage(getRandomGenericReply()); // API yoksa veya ayarlanmamışsa rastgele cevap ver
                }
            }
        }
    }
