...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (!chatBox || !userInput || !sendButton || !uploadButton || !imageUpload || !filterButton || !previewImg || !imagePreview) { 
    ...
    const translations = {
        tr: { 
            ...
            filter_button: "Siyah-Beyaz Yap",
            image_uploaded: "Fotoğraf yüklendi! Filtre uygulamak için butonu kullanabilirsiniz.",
            filter_applied: "Siyah-beyaz filtre uygulandı!",
        ...
        en: { 
            ...
            filter_button: "Make Grayscale",
            image_uploaded: "Photo uploaded! You can apply a filter using the button.",
            filter_applied: "Grayscale filter applied!",
    ...
    function saveData(key, value) {
        try {
            ...
            console.log(`kaydedildi: ${key}=${value}`);
        } catch (e) {
    ...
    function addMessageToChatBoxOnly(text, messageClass) {
        ...
        if (chatBox) {
            ...
            console.error("addMessageToChatBoxOnly içinde chatBox elementi bulunamadı!");
            return;
        }
        scrollToBottom();
    }
    ...
    function getRandomGenericReply() {
        const randomIndex = Math.floor(Math.random() * translations[currentLang].generic_replies.length);
        return translations[currentLang].generic_replies[randomIndex];
    }
    ...
    function updateStaticTexts() {
        ...
        if (uploadButton) uploadButton.textContent = translations[currentLang].upload_button;
        if (filterButton) filterButton.textContent = translations[currentLang].filter_button;
    }
    ...
    function startConversation() {
        console.log("--- startConversation FONKSIYONU ÇALIŞTI! (v163) ---");
        chatHistory = [];
        ...
        if (!currentUserName) {
            addBotMessage(translations[currentLang].ask_name, 100);
    ...
    imageUpload.addEventListener('change', (event) => {
        ...
        if (file) {
            ...
            reader.onload = (e) => {
                ...
                addMessageToChatBoxOnly(translations[currentLang].image_uploaded, 'bot-message');
            };
    ...
    filterButton.addEventListener('click', () => {
        ...
        img.onload = () => {
            ...
            addMessageToChatBoxOnly(translations[currentLang].filter_applied, 'bot-message');
        };
    });
    ...
    function sendMessage() {
        ...
        if (messageText !== "") {
            ...
            if (messageText.startsWith('/')) {
                ...
                switch (command) {
                    case 'help':
                        addBotMessage(translations[currentLang].help_text);
                        break;
                    case 'sponsors':
                        addBotMessage(translations[currentLang].sponsor_list_text);
                        break;
                    case 'rewards':
                        addBotMessage(translations[currentLang].rewards_text);
                        break;
                    case 'myinfo':
                        let cTkM, cMvG, cCsI, cSnN, cCeE, cTgG, cTeE;
                        ...
                        cTkM = loadData(TK_MILES_KEY);
                        cMvG = loadData(MAVI_GSM_KEY);
                        cCsI = loadData(CARREFOURSA_INFO_KEY);
                        cSnN = loadData(SWISSAIR_NO_KEY);
                        cCeE = loadData(CARREFOUR_EU_KEY);
                        cTgG = loadData(TIKTAK_GSM_KEY);
                        cTeE = loadData(TRENDYOL_EMAIL_KEY);
                        ...
                        break;
                    case 'reset':
                        addBotMessage(translations[currentLang].reset_confirm);
                        conversationState = 'awaiting_reset_confirmation';
                        break;
                    case 'turkish':
                        addBotMessage(translations[currentLang].prompt_tk);
                        conversationState = 'awaiting_tk_no';
                        break;
                    case 'mavi':
                        addBotMessage(translations[currentLang].prompt_mavi);
                        conversationState = 'awaiting_mavi_gsm';
                        break;
                    case 'carrefoursa':
                        addBotMessage(translations[currentLang].prompt_carrefoursa);
                        conversationState = 'awaiting_carrefoursa_info';
                        break;
                    case 'swiss':
                        addBotMessage(translations[currentLang].prompt_swissair);
                        conversationState = 'awaiting_swissair_no';
                        break;
                    case 'carrefour_eu':
                        addBotMessage(translations[currentLang].prompt_carrefour_eu);
                        conversationState = 'awaiting_carrefour_eu';
                        break;
                    case 'tiktak':
                        addBotMessage(translations[currentLang].prompt_tiktak);
                        conversationState = 'awaiting_tiktak_gsm';
                        break;
                    case 'trendyol':
                        addBotMessage(translations[currentLang].prompt_trendyol);
                        conversationState = 'awaiting_trendyol_email';
                        break;
                    case 'csa_algida':
                        addBotMessage(translations[currentLang].prompt_csa_algida);
                    ...
                    case 'filter':
                        if (previewImg.src) {
                            ...
                            addBotMessage(translations[currentLang].no_image);
                        }
                        break;
                    default:
                        addBotMessage(translations[currentLang].unknown_command(command));
            ...
            // --- State Handling (Onboarding, Reset, Sponsorlar) ---
                ...
                console.log("Checking allowedSocialMedia:", allowedSocialMedia);
