...
document.addEventListener('DOMContentLoaded', () => {
    console.log(">>> Kyrosil Bot v156 (Lang+TS) çalıştı! <<<");
    ...
    const LANG_KEY = 'kyrosil_userLang'; // Yeni Dil Kaydı
    const TK_MILES_KEY = 'kyrosil_tkMiles'; // ... diğer sponsor keyleri ...
    const MAVI_GSM_KEY = 'kyrosil_maviGsm'; const CARREFOURSA_INFO_KEY = 'kyrosil_carrefoursaInfo'; const SWISSAIR_NO_KEY = 'kyrosil_swissairNo'; const CARREFOUR_EU_KEY = 'kyrosil_carrefourEu'; const TIKTAK_GSM_KEY = 'kyrosil_tiktakGsm'; const TRENDYOL_EMAIL_KEY = 'kyrosil_trendyolEmail';
    ...
    let currentLang = loadData(LANG_KEY) || 'tr'; // Kayıtlı dil yoksa Türkçe başla
    const translations = {
        tr: {
            welcome_new: "Avrupa'nın sponsor markalarla ilk ve tek tam entegre yapay zeka botu KyrosilRewards'a hoş geldin! ✨ Seninle tanışmak istiyorum, adın nedir?",
            ...
            welcome_back: (name) => `Tekrar hoş geldin, ${name}! Mevcut ödül fırsatları için /rewards, sponsor kayıtları için /sponsor-kayit yazabilirsin. Diğer komutlar için /help her zaman yanında. Dil değiştirmek için /lang [en/tr]. Ya da sadece sohbet edelim!`,
            ...
            rewards_text: "Aktif Ödül Fırsatı:\n- CarrefourSA & Algida: 300TL Değerinde Puan Fırsatı! (/carrefoursaxalgida)",
            ...
            generic_reply_api_off: "Şu an sadece belirli komutlara ve selamlaşmalara cevap verebiliyorum ama komutları kullanabilirsin: /help, /sponsor-kayit, /rewards",
        ...
        en: {
            welcome_new: "Welcome to KyrosilRewards, Europe's first and only fully integrated AI bot with sponsor brands! ✨ I'd like to get to know you, what is your name?",
            ...
            welcome_back: (name) => `Welcome back, ${name}! If you're ready for great collaborations and rewards, you can type /sponsor-kayit to see sponsor registration commands or /rewards for current opportunities. /help is always there for other commands. To change language, type /lang [en/tr]. Or let's just chat!`,
            ...
            confirm_tiktak: (gsm) => `Your TikTak (${gsm}) GSM registration has been received. Good luck with your usage!`,
            ...
            rewards_text: "Active Reward Opportunity:\n- CarrefourSA & Algida: 300TL Point Opportunity! (/carrefoursaxalgida)",
            ...
            generic_reply_api_off: "Currently, I can only respond to specific commands and greetings, but you can use commands: /help, /sponsor-kayit, /rewards",
    ...
    let currentUserName, currentSocialMedia, currentSocialUser; // Bunları startConversation içinde yüklüyoruz

    // --- Temel Fonksiyonlar (localStorage, Mesaj Ekleme, Kaydırma) ---
    // ... (saveData, loadData, removeData, removeAllSponsorData fonksiyonları burada) ...
    ...
    function loadData(key){try{const data=localStorage.getItem(key);return data}catch(e){console.error("okuma hatası:",e);return null}}
    ...
    function addMessage(text, role) {
        ...
        // Mesaj içeriğini ve saat damgasını bir wrapper içine alabiliriz (isteğe bağlı)
        const contentWrapper = document.createElement('div');
        contentWrapper.appendChild(paragraph);
        contentWrapper.appendChild(timestampSpan); // Saati paragrafın altına ekle
    ...
    function addMessageToChatBoxOnly(text, messageClass) { // Bu fonksiyon geçmişe EKLEMEZ, sadece gösterir
         const messageElement = document.createElement('div'); messageElement.classList.add('message', messageClass);
         const paragraph = document.createElement('p'); paragraph.textContent = text; messageElement.appendChild(paragraph);
         // Buna da timestamp ekleyebiliriz
         const timestamp = new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });
         ...
         paragraph.appendChild(timestampSpan); // Saati direkt P içine ekleyelim bunda

         if (chatBox) { chatBox.appendChild(messageElement); } else { console.error("addMessageToChatBoxOnly içinde chatBox elementi bulunamadı!"); return; } scrollToBottom();
    }
    function addBotMessage(text, delay = 600) { setTimeout(() => { addMessage(text, 'model'); }, delay); }
    function scrollToBottom() { if (chatBox) { chatBox.scrollTop = chatBox.scrollHeight; } }
    function getRandomGenericReply() { const randomIndex = Math.floor(Math.random() * translations[currentLang].generic_replies.length); return translations[currentLang].generic_replies[randomIndex]; }

    // === Gemini API Fonksiyonu ===
    async function getGeminiResponse() {
        if (chatHistory.length === 0 || chatHistory[chatHistory.length - 1].role !== 'user') { return; }
        addMessageToChatBoxOnly("...", 'bot-message');
        const thinkingMessageElement = chatBox.lastElementChild;
        const payload = { contents: chatHistory };
        try {
            if (!GEMINI_API_KEY || GEMINI_API_KEY === 'SENIN_API_ANAHTARIN_BURAYA') { throw new Error(translations[currentLang].api_fail_error("API Anahtarı ayarlanmamış veya geçersiz.")); } // Hata mesajını çeviriden al
            ...
            let botReply = translations[currentLang].api_fail_generic; // Çeviriden al
            ...
            else if (data.promptFeedback && data.promptFeedback.blockReason){ botReply = translations[currentLang].content_blocked(data.promptFeedback.blockReason); console.error("API İçerik Engeli:", data.promptFeedback); } // Çeviriden al
            else { console.error("Beklenmeyen API cevap formatı:", data); }
            if (thinkingMessageElement && thinkingMessageElement.classList.contains('bot-message') && thinkingMessageElement.textContent.startsWith("...")) { thinkingMessageElement.remove(); }
            addMessage(botReply, 'model');
        } catch (error) {
            console.error("Gemini API isteği başarısız:", error);
            if (thinkingMessageElement && thinkingMessageElement.classList.contains('bot-message') && thinkingMessageElement.textContent.startsWith("...")) { thinkingMessageElement.remove(); }
            addMessageToChatBoxOnly(translations[currentLang].api_fail_error(error.message), 'bot-message'); // Hata mesajını çeviriden al (geçmişe ekleme)
        }
    }

    // === Konuşmayı Başlatma Fonksiyonu (Dil Desteği Eklendi) ===
    function startConversation() {
        console.log("--- startConversation FONKSIYONU ÇALIŞTI! ---");
        chatHistory = [];
        // İlk mesajı aktif dile göre ekle
        addMessageToChatBoxOnly(translations[currentLang].welcome_new.split("✨")[0] + "✨", 'bot-message'); // Sadece ilk kısmı alalım, soru sonra gelecek.
    ...
    // === Mesaj Gönderme Mantığı (Dil Desteği ve BUG FIX Eklendi) ===
    function sendMessage() {
        ...
        if (messageText !== "") {
            ...
            // Farklı durumları işle
            if (conversationState === 'awaiting_name') {
                currentUserName = messageText; saveData(USER_NAME_KEY, currentUserName);
                // Tekrar startConversation çağırmak yerine direkt sonraki adımı sor (BUG FIX)
                 addBotMessage(translations[currentLang].ask_social(currentUserName), 100);
                ...
                if (allowedSocialMedia.includes(lowerCaseInput)) {
                    currentSocialMedia = lowerCaseInput; saveData(SOCIAL_MEDIA_KEY, currentSocialMedia);
                     // Tekrar startConversation çağırmak yerine direkt sonraki adımı sor (BUG FIX)
                    addBotMessage(translations[currentLang].ask_username(currentSocialMedia), 100);
                    conversationState = 'awaiting_username';
                } else {
                    addBotMessage(translations[currentLang].prompt_social);
                }
            } else if (conversationState === 'awaiting_username') {
                currentSocialUser = messageText; saveData(SOCIAL_USER_KEY, currentSocialUser);
                addBotMessage(translations[currentLang].onboarding_complete(currentUserName));
                 // Tekrar startConversation çağırmak yerine direkt hoşgeldin mesajını ver (BUG FIX)
                 addBotMessage(translations[currentLang].welcome_back(currentUserName), 100);
                conversationState = 'idle';
            } else if (conversationState === 'awaiting_reset_confirmation') {
                const lowerCaseInput = messageText.toLowerCase();
                if (lowerCaseInput === 'evet' || lowerCaseInput === 'yes') { // İngilizce 'yes'i de ekleyelim
                    ...
                    addBotMessage(translations[currentLang].reset_done);
                    conversationState = 'idle';
                    setTimeout(startConversation, 800);
                } else { addBotMessage(translations[currentLang].reset_cancel); conversationState = 'idle'; }

            // Sponsor State Handlers (Dil Desteği Eklendi)
            ...
            } else if (conversationState === 'awaiting_swissair_no') { const swissNo = messageText; saveData(SWISSAIR_NO_KEY, swissNo); addBotMessage(translations[currentLang].confirm_swissair(no)); conversationState = 'idle';
            ...
            } else if (conversationState === 'awaiting_tiktak_gsm') { const tiktakGsm = messageText; saveData(TIKTAK_GSM_KEY, tiktakGsm); addBotMessage(translations[currentLang].confirm_tiktak(gsm)); conversationState = 'idle';
            } else if (conversationState === 'awaiting_trendyol_email') { const trendyolMail = messageText; saveData(TRENDYOL_EMAIL_KEY, trendyolMail); addBotMessage(translations[currentLang].confirm_trendyol(mail)); conversationState = 'idle';
            ...
            // === Diğer Sponsor Handler'lar bitti ===
                ...
                if (messageText.startsWith('/')) {
                    ...
                    switch (command) {
                        ...
                        case 'bilgilerim':
                            let currentTkMiles,currentMaviGsm,currentCarrefoursaInfo,currentSwissairNo,currentCarrefourEu,currentTiktakGsm,currentTrendyolEmail; currentUserName=loadData(USER_NAME_KEY);currentSocialMedia=loadData(SOCIAL_MEDIA_KEY);currentSocialUser=loadData(SOCIAL_USER_KEY);currentTkMiles=loadData(TK_MILES_KEY);currentMaviGsm=loadData(MAVI_GSM_KEY);currentCarrefoursaInfo=loadData(CARREFOURSA_INFO_KEY);currentSwissairNo=loadData(SWISSAIR_NO_KEY);currentCarrefourEu=loadData(CARREFOUR_EU_KEY);currentTiktakGsm=loadData(TIKTAK_GSM_KEY);currentTrendyolEmail=loadData(TRENDYOL_EMAIL_KEY);
                        ...
                        // Sponsor komutları (Promptlar çeviriden gelecek)
                        ...
                        // === YENİ: Dil Değiştirme Komutu ===
                        case 'lang':
                            if (args.length > 0 && (args[0] === 'en' || args[0] === 'tr')) {
                                currentLang = args[0];
                                saveData(LANG_KEY, currentLang); // Seçimi kaydet
                                addBotMessage(translations[currentLang].lang_set_en === "Language set to English." ? translations[currentLang].lang_set_en : translations[currentLang].lang_set_tr); // Doğru dilde onay ver
                            } else {
                    ...
                    let greetingHandled = true;
                     // Selamlaşma cevapları da çeviriden gelmeli (basit olanlar için şimdilik direkt yazdık)
                     if (lowerCaseInput === 'merhaba' || lowerCaseInput === 'selam' || lowerCaseInput === 'hello' || lowerCaseInput === 'hi') {
                        addBotMessage(translations[currentLang].greeting_hello(currentUserName));
                    } else if (lowerCaseInput === 'naber' || lowerCaseInput === 'nasılsın' || lowerCaseInput.includes('how are you')) {
                        addBotMessage(translations[currentLang].greeting_how_are_you);
                    ...
                    if (!greetingHandled) {
                        if (typeof GEMINI_API_KEY !== 'undefined' && GEMINI_API_KEY && GEMINI_API_KEY !== 'SENIN_API_ANAHTARIN_BURAYA') {
                             ...
                             addBotMessage(getRandomGenericReply()); // API yoksa rastgele cevap (bunu da çevirebiliriz)
                        }
                    }
                }
            }
    ...
    // Dil tercihini yükle (veya varsayılanı tr yap)
