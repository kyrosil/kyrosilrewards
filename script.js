...
document.addEventListener('DOMContentLoaded', () => {
    console.log(">>> Kyrosil Bot v161 FINAL FIX (DOMContentLoaded) çalıştı! <<<"); // Versiyon adı
    ...
    let currentUserName, currentSocialMedia, currentSocialUser; // Bunları fonksiyon içinde yüklüyoruz/güncelliyoruz
    ...
    function startConversation() {
        console.log("--- startConversation FONKSIYONU ÇALIŞTI! (v161) ---"); chatHistory = [];
        ...
        if (currentLang === 'tr') { initialWelcome += "\n(For English, type /lang en)"; } else { initialWelcome += "\n(Türkçe için /lang tr yazın)"; }
         addMessageToChatBoxOnly(initialWelcome, 'bot-message'); // Sadece ekrana yaz
    ...
    // --- Mesaj Gönderme Ana Mantığı (DÜZELTİLMİŞ) ---
    function sendMessage() {
        ...
        if (messageText !== "") {
            ...
            if (messageText.startsWith('/')) {
                ...
                switch (command) {
                    ...
                    case 'lang':
                        if (args.length > 0 && (args[0] === 'en' || args[0] === 'tr')) {
                             currentLang = args[0]; saveData(LANG_KEY, currentLang); addBotMessage(currentLang === 'en' ? translations.en.lang_set_en : translations.tr.lang_set_tr);
                ...
                // Bilinen komut işlendiyse fonksiyondan çık
            ...
            // Eğer mesaj komut değilse, o zaman state'e göre işle
             ...
             // Eğer 'idle' durumundaysak ve mesaj komut veya state'e özel değilse, selamlaşma veya API çağrısı yap
                  ...
                  if (!greetingHandled) { // Selamlaşma değilse
