...
document.addEventListener('DOMContentLoaded', () => {
    console.log(">>> Kyrosil Bot FINAL (v165) çalıştı! <<<"); // Son versiyon
    ...
    async function getGeminiResponse() {
        ...
        addMessageToChatBoxOnly("...", 'bot-message'); const thinkingMessageElement = chatBox.lastElementChild; const payload = { contents: chatHistory };
        try { if (!GEMINI_API_KEY || GEMINI_API_KEY === 'SENIN_API_ANAHTARIN_BURAYA') { throw new Error(translations[currentLang].api_fail_error("API Anahtarı ayarlanmamış veya geçersiz.")); } const response = await fetch(API_URL, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) }); if (!response.ok) { const errorData = await response.json(); throw new Error(`API Hatası: ${response.status} - ${errorData?.error?.message || response.statusText}`); } const data = await response.json(); let botReply = translations[currentLang].api_fail_generic; if (data.candidates && data.candidates.length > 0 && data.candidates[0].content && data.candidates[0].content.parts && data.candidates[0].content.parts.length > 0) { botReply = data.candidates[0].content.parts[0].text; } else if (data.promptFeedback && data.promptFeedback.blockReason){ botReply = translations[currentLang].content_blocked(data.promptFeedback.blockReason); console.error("API İçerik Engeli:", data.promptFeedback); } else { console.error("Beklenmeyen API cevap formatı:", data); } if (thinkingMessageElement && thinkingMessageElement.classList.contains('bot-message') && thinkingMessageElement.textContent.startsWith("...")) { thinkingMessageElement.remove(); } addMessage(botReply, 'model');
        } catch (error) { console.error("Gemini API isteği başarısız:", error); if (thinkingMessageElement && thinkingMessageElement.classList.contains('bot-message') && thinkingMessageElement.textContent.startsWith("...")) { thinkingMessageElement.remove(); } addMessage(translations[currentLang].api_fail_error(error.message), 'model'); }
    }

    // --- Konuşmayı Başlatma Fonksiyonu ---
    function startConversation() {
        console.log("--- startConversation FONKSIYONU ÇALIŞTI! (v163) ---"); chatHistory = [];
        let initialWelcome = translations[currentLang].welcome_new; if (currentLang === 'tr') { initialWelcome += "\n(For English, type /lang en)"; } else { initialWelcome += "\n(Türkçe için /lang tr yazın)"; } addMessageToChatBoxOnly(initialWelcome, 'bot-message');
        ...
        if (messageText !== "") {
            ...
            if (messageText.startsWith('/')) {
                ...
                switch (command) {
                    ...
                    case 'swiss': addBotMessage(translations[currentLang].prompt_swissair); conversationState = 'awaiting_swissair_no'; break;
                    case 'carrefour_eu': addBotMessage(translations[currentLang].prompt_carrefour_eu); conversationState = 'awaiting_carrefour_eu'; break;
                    ...
                    case 'csa_algida': addBotMessage(translations[currentLang].prompt_csa_algida); conversationState = 'awaiting_carrefour_no'; break; // Algida için ayrı key/state kullanmadık, eski carrefour_no state'i kaldı
                    case 'lang':
                        if (args.length > 0 && (args[0] === 'en' || args[0] === 'tr')) {
                             currentLang = args[0]; saveData(LANG_KEY, currentLang); addBotMessage(currentLang === 'en' ? translations.en.lang_set_en : translations.tr.lang_set_tr); updateStaticTexts();
                             // Mevcut soruyu tekrar sor
                             if (conversationState === 'awaiting_name') { addBotMessage(translations[currentLang].ask_name, 100); } else if (conversationState === 'awaiting_social') { addBotMessage(translations[currentLang].ask_social(currentUserName), 100); } else if (conversationState === 'awaiting_username') { addBotMessage(translations[currentLang].ask_username(currentSocialMedia), 100); }
                 ...
                 // Bilinen ve state değiştirmeyen komut işlendiyse veya state değiştiren komut işlendiyse çık
                if (commandHandled) {
                    console.log(`Komut islendi: ${command}`);
                    return;
                }
            }
            ...
            else if (conversationState === 'awaiting_name') { currentUserName = messageText; saveData(USER_NAME_KEY, currentUserName); addBotMessage(translations[currentLang].ask_social(currentUserName), 100); conversationState = 'awaiting_social'; }
            else if (conversationState === 'awaiting_social') {
                console.log("Checking allowedSocialMedia:", allowedSocialMedia); // <<< DEBUG LOGU
                const lci = messageText.toLowerCase();
                if (allowedSocialMedia.includes(lci)) { currentSocialMedia = lci; saveData(SOCIAL_MEDIA_KEY, currentSocialMedia); addBotMessage(translations[currentLang].ask_username(currentSocialMedia), 100); conversationState = 'awaiting_username'; }
                else { addBotMessage(translations[currentLang].prompt_social); }
            }
            ...
            else if (conversationState === 'awaiting_carrefour_no') { const enteredNumber=messageText;saveData(CSA_ALGIDA_KEY, enteredNumber); addBotMessage(translations[currentLang].confirm_csa_algida(enteredNumber));conversationState='idle'; } // Algida bilgisini de kaydedelim
            ...
            // Eğer 'idle' durumundaysak ve mesaj komut veya state'e özel değilse, selamlaşma veya API
                 ...
                 const lowerCaseInput = messageText.toLowerCase(); let greetingHandled = true;
                 ...
                 if (!greetingHandled) {
                     if (typeof GEMINI_API_KEY !== 'undefined' && GEMINI_API_KEY && GEMINI_API_KEY !== 'SENIN_API_ANAHTARIN_BURAYA') { getGeminiResponse(); }
                     else { addBotMessage(getRandomGenericReply()); }
                 } else { console.log("Selamlaşma işlendi, API çağrısı atlanıyor."); }
            }
        }
    }
    ...
    updateStaticTexts(); // Statik metinleri ayarla
