...
document.addEventListener('DOMContentLoaded', () => {
    console.log(">>> Kyrosil Bot v163 FINAL (English Commands) çalıştı! <<<"); // Versiyon adı güncellendi
    ...
    // Dil Ayarları ve Metinler (Komut adları güncellendi)
    let currentLang = loadData(LANG_KEY) || 'tr';
    const translations = {
        tr: {
             welcome_new:"Avrupa'nın sponsor markalarla ilk ve tek tam entegre yapay zeka botu KyrosilRewards'a hoş geldin! ✨",ask_name:"Seninle tanışmak istiyorum, adın nedir?",ask_social:(name)=>`Tekrar merhaba ${name}! Bizi hangi sosyal medya platformundan takip ediyorsun? (Instagram, EU Portal, X, Tiktok)`,ask_username:(social)=>`Harika, ${social} üzerinden takip etmene sevindim! Oradaki kullanıcı adın nedir?`,onboarding_complete:(name)=>`Teşekkürler ${name}! Tüm bilgilerin kaydedildi.`,welcome_back:(name)=>`Tekrar hoş geldin, ${name}! Mevcut ödül fırsatları için /rewards, sponsor kayıtları için /sponsors yazabilirsin. Diğer komutlar için /help her zaman yanında. Dil değiştirmek için /lang [en/tr]. Ya da sadece sohbet edelim!`, // /sponsor-kayit -> /sponsors
             prompt_social:"Lütfen listedeki platformlardan birini yazar mısın? (Instagram, EU Portal, X, Tiktok)",reset_confirm:"Emin misin? Kayıtlı tüm bilgilerin (tanışma ve sponsor) silinecek. Onaylamak için 'Evet' yaz.",reset_done:"Tüm kayıtlı bilgilerin (tanışma ve sponsor) silindi.",reset_cancel:"İşlem iptal edildi.",
             prompt_tk:"Lütfen Miles&Smiles üyelik numaranızı (TK ile başlayan) girin:", confirm_tk:(tkNo)=>`Miles&Smiles (${tkNo}) kaydınız alındı. Özel teklifler için takipte kalın!`,
             prompt_mavi:"Lütfen Mavi Kartuş Kart'a kayıtlı GSM numaranızı girin:", confirm_mavi:(gsm)=>`Mavi Kartuş (${gsm}) GSM kaydınız alındı. Kampanyalardan haberdar edileceksiniz!`,
             prompt_carrefoursa:"Lütfen CarrefourSA kart numaranızı veya kayıtlı GSM numaranızı girin:", confirm_carrefoursa:(info)=>`CarrefourSA (${info}) bilginiz kaydedildi. İlgili kampanyalar hakkında bilgi verilecek!`,
             prompt_swissair:"Lütfen Swiss Air özel yolcu programı numaranızı girin:", confirm_swissair:(no)=>`Swiss Air (${no}) yolcu programı kaydınız alındı. Uçuşlarınızda başarılar!`,
             prompt_carrefour_eu:"Lütfen Carrefour Card (Avrupa) numaranızı girin:", confirm_carrefour_eu:(no)=>`Carrefour Avrupa (${no}) kart bilginiz kaydedildi. Bölgesel kampanyalar için takipte kalın!`,
             prompt_tiktak:"Lütfen TikTak'a kayıtlı GSM numaranızı girin:", confirm_tiktak:(gsm)=>`TikTak (${gsm}) GSM kaydınız alındı. Kullanımlarınızda bol şans!`,
             prompt_trendyol:"Lütfen Trendyol'a kayıtlı e-posta adresinizi girin:", confirm_trendyol:(mail)=>`Trendyol (${mail}) e-posta adresiniz kaydedildi. Özel indirimler için hesabınızı kontrol edin!`,
             prompt_csa_algida:"Lütfen CarrefourSA Kart'a kayıtlı GSM numaranızı veya Kart numaranızı girin (Algida Kampanyası):", confirm_csa_algida:(num)=>`Katılımınız alındı! Girdiğiniz numara (${num}) için kısa süre içerisinde otomatik sistemlerimiz kartınıza 300 TL değerindeki Algida puanını tanımlayacaktır.`,
             help_text:"Kullanılabilir Komutlar:\n/help - Yardım.\n/myinfo - Bilgilerini gösterir.\n/reset - Bilgilerini siler.\n/sponsors - Sponsor kayıt komutlarını listeler.\n/rewards - Ödül fırsatları.\n/lang [en/tr] - Dil değiştirir.", // Komutlar İngilizce
             sponsor_list_text:"Sponsor Kayıt Komutları:\n/turkish - Miles&Smiles No Kaydı\n/mavi - Mavi GSM Kaydı\n/carrefoursa - C.SA Kart/GSM Kaydı\n/swiss - Swiss Air Yolcu No Kaydı\n/carrefour_eu - Carrefour Avrupa Kart Kaydı\n/tiktak - TikTak GSM Kaydı\n/trendyol - Trendyol E-posta Kaydı", // Komutlar İngilizce
             rewards_text:"Aktif Ödül Fırsatı:\n- CarrefourSA & Algida: 300TL Değerinde Puan Fırsatı! (/csa_algida)", // Komut İngilizce
             my_info_title_basic:"--- Temel Bilgilerin ---",my_info_name:"İsim:",my_info_platform:"Platform:",my_info_username:"K.Adı:",my_info_title_sponsor:"--- Sponsor Kayıtların ---",my_info_sponsor_tk:"THY (M&S):",my_info_sponsor_mavi:"Mavi (GSM):",my_info_sponsor_csa:"C.SA (Kart/GSM):",my_info_sponsor_swiss:"Swiss Air (No):",my_info_sponsor_ceu:"C. EU (Kart):",my_info_sponsor_tiktak:"TikTak (GSM):",my_info_sponsor_trendyol:"Trendyol (Mail):",not_registered:"Kaydedilmemiş",not_registered_short:"-",unknown_command:(cmd)=>`Bilinmeyen komut: "${cmd}". Yardım için /help yazabilirsin.`,greeting_hello:(name)=>`Merhaba ${name||''}!`,greeting_how_are_you:"İyiyim, sorduğun için teşekkürler! Sen nasılsın?",generic_reply_api_off:"Şu an sadece belirli komutlara ve selamlaşmalara cevap verebiliyorum ama komutları kullanabilirsin: /help, /sponsors, /rewards",api_fail_generic:"Üzgünüm, bir cevap alamadım.",api_fail_error:(err)=>`Üzgünüm, şu an sana cevap veremiyorum. Hata: ${err}`,content_blocked:(reason)=>`İçerik güvenlik nedeniyle engellendi: ${reason}`,model_loading:"Model şu an yükleniyor, lütfen birkaç saniye sonra tekrar deneyin...",lang_set_tr:"Dil Türkçe olarak ayarlandı.",lang_set_en:"Language set to English.",lang_fail:"Desteklenmeyen dil kodu. Lütfen 'en' veya 'tr' kullanın.",generic_replies:["Anladım.","Hmm, peki.","İlginç.","Tamamdır.","Devam et...","Peki."], input_placeholder:"Mesajınızı yazın...", send_button_text:"Gönder"},
        en: { welcome_new:"Welcome to KyrosilRewards, Europe's first and only fully integrated AI bot with sponsor brands! ✨",ask_name:"I'd like to get to know you, what is your name?",ask_social:(name)=>`Welcome back ${name}! Which social media platform do you follow us from? (Instagram, EU Portal, X, Tiktok)`,ask_username:(social)=>`Great, glad you follow us via ${social}! What is your username there?`,onboarding_complete:(name)=>`Thanks ${name}! All your information has been saved.`,welcome_back:(name)=>`Welcome back, ${name}! If you're ready for great collaborations and rewards, you can type /rewards for current opportunities or /sponsors to see sponsor registration commands. /help is always there for other commands. To change language, type /lang [en/tr]. Or let's just chat!`, // /sponsor-kayit -> /sponsors
             prompt_social:"Please type one of the platforms from the list (Instagram, EU Portal, X, Tiktok)",reset_confirm:"Are you sure? All your registered information (onboarding and sponsor) will be deleted. Type 'Yes' to confirm.",reset_done:"All your registered information (onboarding and sponsor) has been deleted.",reset_cancel:"Operation cancelled.",
             prompt_tk:"Please enter your Miles&Smiles membership number (starting with TK):", confirm_tk:(tkNo)=>`Your Miles&Smiles (${tkNo}) registration has been received. Stay tuned for special offers!`,
             prompt_mavi:"Please enter your GSM number registered to Mavi Kartuş Kart:", confirm_mavi:(gsm)=>`Your Mavi Kartuş (${gsm}) GSM registration has been received. You will be informed about campaigns!`,
             prompt_carrefoursa:"Please enter your CarrefourSA card number or registered GSM number:", confirm_carrefoursa:(info)=>`Your CarrefourSA (${info}) information has been saved. You will be informed about relevant campaigns!`,
             prompt_swissair:"Please enter your Swiss Air special passenger program number:", confirm_swissair:(no)=>`Your Swiss Air (${no}) passenger program registration has been received. Have a good flight!`,
             prompt_carrefour_eu:"Please enter your Carrefour Card (Europe) number:", confirm_carrefour_eu:(no)=>`Your Carrefour Europe (${no}) card information has been saved. Stay tuned for regional campaigns!`,
             prompt_tiktak:"Please enter your GSM number registered to TikTak:", confirm_tiktak:(gsm)=>`Your TikTak (${gsm}) registration has been received. Good luck with your usage!`,
             prompt_trendyol:"Please enter your e-mail address registered to Trendyol:", confirm_trendyol:(mail)=>`Your Trendyol (${mail}) e-mail address has been saved. Check your account for special discounts!`,
             prompt_csa_algida:"Please enter your CarrefourSA Card registered GSM number or Card number (Algida Campaign):", confirm_csa_algida:(num)=>`Your participation is received! Our automated systems will credit the 300 TL worth of Algida points to your card for the number (${num}) shortly.`,
             help_text:"Available Commands:\n/help - Show this help message.\n/myinfo - Show your registered info.\n/reset - Delete all your registered info.\n/sponsors - List sponsor registration commands.\n/rewards - Show active reward opportunities.\n/lang [en/tr] - Change language.", // Komutlar İngilizce
             sponsor_list_text:"Sponsor Registrations:\n/turkish - Miles&Smiles No\n/mavi - Mavi GSM\n/carrefoursa - C.SA Card/GSM\n/swiss - Swiss Air No\n/carrefour_eu - Carrefour Europe Card\n/tiktak - TikTak GSM\n/trendyol - Trendyol E-mail", // Komutlar İngilizce
             rewards_text:"Active Reward Opportunity:\n- CarrefourSA & Algida: 300TL Point Opportunity! (/csa_algida)", // Komut İngilizce
             my_info_title_basic:"--- Your Basic Info ---",my_info_name:"Name:",my_info_platform:"Platform:",my_info_username:"Username:",my_info_title_sponsor:"--- Your Sponsor Registrations ---",my_info_sponsor_tk:"THY (M&S):",my_info_sponsor_mavi:"Mavi (GSM):",my_info_sponsor_csa:"C.SA (Card/GSM):",my_info_sponsor_swiss:"Swiss Air (No):",my_info_sponsor_ceu:"C. EU (Card):",my_info_sponsor_tiktak:"TikTak (GSM):",my_info_sponsor_trendyol:"Trendyol (Mail):",not_registered:"Not Registered",not_registered_short:"-",unknown_command:(cmd)=>`Unknown command: "${cmd}". Type /help for assistance.`,greeting_hello:(name)=>`Hello ${name||''}!`,greeting_how_are_you:"I'm doing well, thanks for asking! How are you?",generic_reply_api_off:"Currently, I can only respond to specific commands and greetings, but you can use commands: /help, /sponsors, /rewards",api_fail_generic:"Sorry, I couldn't get a response.",api_fail_error:(err)=>`Sorry, I can't answer you right now. Error: ${err}`,content_blocked:(reason)=>`Content blocked due to safety reasons: ${reason}`,model_loading:"The model is currently loading, please try again in a few seconds...",lang_set_tr:"Dil Türkçe olarak ayarlandı.",lang_set_en:"Language set to English.",lang_fail:"Unsupported language code. Please use 'en' or 'tr'.",generic_replies:["I see.","Hmm, okay.","Interesting point.","Noted.","Alright.","You can continue...","Okay, any other topic?"], input_placeholder:"Type your message...", send_button_text:"Send" }
    };

    // --- Değişkenler ---
    let chatHistory = []; const MAX_HISTORY_LENGTH = 6;
    let currentUserName, currentSocialMedia, currentSocialUser;

    // --- Temel Fonksiyonlar ---
     function saveData(key, value){try{localStorage.setItem(key, value);console.log(`kaydedildi: ${key}=${value}`)}catch(e){console.error("kayıt hatası:",e)}};function loadData(key){try{const data=localStorage.getItem(key);return data}catch(e){console.error("okuma hatası:",e);return null}};function removeData(key){try{localStorage.removeItem(key);console.log(`silindi: ${key}`)}catch(e){console.error("silme hatası:",e)}};function removeAllSponsorData(){removeData(TK_MILES_KEY);removeData(MAVI_GSM_KEY);removeData(CARREFOURSA_INFO_KEY);removeData(SWISSAIR_NO_KEY);removeData(CARREFOUR_EU_KEY);removeData(TIKTAK_GSM_KEY);removeData(TRENDYOL_EMAIL_KEY);removeData('kyrosil_csaAlgidaInfo'); /* Algida key'i de silelim */};function addMessage(text, role) { if (!text || text.trim() === '') return; const messageClass = role === 'user' ? 'user-message' : 'bot-message'; const messageElement = document.createElement('div'); messageElement.classList.add('message', messageClass); const paragraph = document.createElement('p'); paragraph.textContent = text; const timestamp = new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }); const timestampSpan = document.createElement('span'); timestampSpan.classList.add('timestamp'); timestampSpan.textContent = timestamp; const contentWrapper = document.createElement('div'); contentWrapper.appendChild(paragraph); contentWrapper.appendChild(timestampSpan); messageElement.appendChild(contentWrapper); if (chatBox) { chatBox.appendChild(messageElement); } else { console.error("addMessage içinde chatBox elementi bulunamadı!"); return; } if(role === 'user' || role === 'model') { chatHistory.push({ role: role, parts: [{ text: text }] }); if (chatHistory.length > MAX_HISTORY_LENGTH) { chatHistory = chatHistory.slice(chatHistory.length - MAX_HISTORY_LENGTH); } } scrollToBottom(); }; function addMessageToChatBoxOnly(text, messageClass) { if (!text || text.trim() === '') return; const messageElement = document.createElement('div'); messageElement.classList.add('message', messageClass); const paragraph = document.createElement('p'); paragraph.textContent = text; const timestamp = new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }); const timestampSpan = document.createElement('span'); timestampSpan.classList.add('timestamp'); timestampSpan.textContent = timestamp; const contentWrapper = document.createElement('div'); contentWrapper.appendChild(paragraph); contentWrapper.appendChild(timestampSpan); messageElement.appendChild(contentWrapper); if (chatBox) { chatBox.appendChild(messageElement); } else { console.error("addMessageToChatBoxOnly içinde chatBox elementi bulunamadı!"); return; } scrollToBottom(); }; function addBotMessage(text, delay = 600) { setTimeout(() => { addMessage(text, 'model'); }, delay); }; function scrollToBottom() { if (chatBox) { chatBox.scrollTop = chatBox.scrollHeight; } }; function getRandomGenericReply() { const randomIndex = Math.floor(Math.random() * translations[currentLang].generic_replies.length); return translations[currentLang].generic_replies[randomIndex]; };
    ...
    function updateStaticTexts() {
        ...
        if (sendButton) sendButton.textContent = translations[currentLang].send_button_text;
        // const chatTitle = document.getElementById('chat-title');
        // if (chatTitle) chatTitle.textContent = translations[currentLang].chat_title_text || "Kyrosil Rewards Bot"; // Başlık için de eklenebilir
    }
    ...
    function startConversation() {
        ...
        if (currentLang === 'tr') { initialWelcome += "\n(For English, type /lang en)"; } else { initialWelcome += "\n(Türkçe için /lang tr yazın)"; }
         addMessageToChatBoxOnly(initialWelcome, 'bot-message');
    ...
    // --- Mesaj Gönderme Ana Mantığı (İNGİLİZCE KOMUTLAR) ---
    function sendMessage() {
        ...
        if (messageText !== "") {
            ...
            if (messageText.startsWith('/')) {
                const parts = messageText.substring(1).toLowerCase().trim().split(' '); const command = parts[0]; const args = parts.slice(1); let commandHandled = true;
                // KOMUTLARI İNGİLİZCE OLARAK KONTROL ET
                switch (command) {
                    case 'help': addBotMessage(translations[currentLang].help_text); break;
                    case 'sponsors': // /sponsor-kayit yerine
                         addBotMessage(translations[currentLang].sponsor_list_text); break;
                    case 'rewards': addBotMessage(translations[currentLang].rewards_text); break;
                    case 'myinfo': // /bilgilerim yerine
                        let cTkM, cMvG, cCsI, cSnN, cCeE, cTgG, cTeE; // Yeni değişken adları (scope için)
                        currentUserName=loadData(USER_NAME_KEY);currentSocialMedia=loadData(SOCIAL_MEDIA_KEY);currentSocialUser=loadData(SOCIAL_USER_KEY);cTkM=loadData(TK_MILES_KEY);cMvG=loadData(MAVI_GSM_KEY);cCsI=loadData(CARREFOURSA_INFO_KEY);cSnN=loadData(SWISSAIR_NO_KEY);cCeE=loadData(CARREFOUR_EU_KEY);cTgG=loadData(TIKTAK_GSM_KEY);cTeE=loadData(TRENDYOL_EMAIL_KEY);
                    ...
                    // SPONSOR KOMUTLARI DA İNGİLİZCE
                    case 'turkish': addBotMessage(translations[currentLang].prompt_tk); conversationState = 'awaiting_tk_no'; break;
                    ...
                    case 'swiss': addBotMessage(translations[currentLang].prompt_swissair); conversationState = 'awaiting_swissair_no'; break; // swissair -> swiss
                    case 'carrefour_eu': addBotMessage(translations[currentLang].prompt_carrefour_eu); conversationState = 'awaiting_carrefour_eu'; break; // carrefour -> carrefour_eu
                    ...
                    case 'csa_algida': addBotMessage(translations[currentLang].prompt_csa_algida); conversationState = 'awaiting_carrefour_no'; break; // carrefoursaxalgida -> csa_algida
                    // Dil değiştirme komutu
                    case 'lang':
                        if (args.length > 0 && (args[0] === 'en' || args[0] === 'tr')) {
                             currentLang = args[0]; saveData(LANG_KEY, currentLang);
                             addBotMessage(currentLang === 'en' ? translations.en.lang_set_en : translations.tr.lang_set_tr);
                             updateStaticTexts(); // Buton vs güncelle
                             // Mevcut soruyu tekrar sor
                             if (conversationState === 'awaiting_name') { addBotMessage(translations[currentLang].ask_name, 100); }
                             else if (conversationState === 'awaiting_social') { addBotMessage(translations[currentLang].ask_social(currentUserName), 100); }
                             else if (conversationState === 'awaiting_username') { addBotMessage(translations[currentLang].ask_username(currentSocialMedia), 100); }
                        } else { addBotMessage(translations[currentLang].lang_fail); }
                        break;
                    default: addBotMessage(translations[currentLang].unknown_command(command)); commandHandled = false;
                }
                if (commandHandled) return; // Komut işlendiyse çık
            }

            // --- State Handling (Onboarding, Reset, Sponsorlar) ---
            // Komut değilse state'e göre işle... (Bu kısım aynı kalıyor, sadece state'e göre prompt/confirm mesajları çeviriden geliyor)
            ...
            else if (conversationState === 'awaiting_swissair_no') { const swissNo=messageText;saveData(SWISSAIR_NO_KEY,swissNo); addBotMessage(translations[currentLang].confirm_swissair(swissNo));conversationState='idle'; }
            ...
            else if (conversationState === 'awaiting_tiktak_gsm') { const tiktakGsm=messageText;saveData(TIKTAK_GSM_KEY,tiktakGsm); addBotMessage(translations[currentLang].confirm_tiktak(tiktakGsm));conversationState='idle'; }
            else if (conversationState === 'awaiting_trendyol_email') { const trendyolMail=messageText;saveData(TRENDYOL_EMAIL_KEY,trendyolMail); addBotMessage(translations[currentLang].confirm_trendyol(trendyolMail));conversationState='idle'; }
            ...
            // Idle state, komut değil, selamlaşma değil -> API
    ...
    updateStaticTexts(); // Sayfa ilk yüklendiğinde statik metinleri ayarla
