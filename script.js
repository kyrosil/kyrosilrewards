...
document.addEventListener('DOMContentLoaded', () => {
    console.log(">>> Kyrosil Bot v159 (Lang Hint + TS Fix) çalıştı! <<<"); // Bu versiyon numarasını takip edebilirsin
    ...
    function startConversation() {
        ...
        if (currentLang === 'tr') {
         ...
         addMessageToChatBoxOnly(initialWelcome, 'bot-message'); // Geçmişe ekleme
         // =============================
            ...
            if (conversationState === 'awaiting_name') { currentUserName = messageText; saveData(USER_NAME_KEY, currentUserName); addBotMessage(translations[currentLang].ask_social(currentUserName), 100); conversationState = 'awaiting_social'; }
                ...
                if (messageText.startsWith('/')) {
                    ...
                    switch (command) {
                        ...
                        case 'bilgilerim':
                            let cTk,cMg,cCsi,cSn,cCe,cTg,cTe; currentUserName=loadData(USER_NAME_KEY);currentSocialMedia=loadData(SOCIAL_MEDIA_KEY);currentSocialUser=loadData(SOCIAL_USER_KEY);cTk=loadData(TK_MILES_KEY);cMg=loadData(MAVI_GSM_KEY);cCsi=loadData(CARREFOURSA_INFO_KEY);cSn=loadData(SWISSAIR_NO_KEY);cCe=loadData(CARREFOUR_EU_KEY);cTg=loadData(TIKTAK_GSM_KEY);cTe=loadData(TRENDYOL_EMAIL_KEY);
                        ...
                        case 'lang':
                            if (args.length > 0 && (args[0] === 'en' || args[0] === 'tr')) {
                                ...
                                // Dil değişince başlangıç mesajını tekrar verelim mi? Şimdilik vermeyelim.
