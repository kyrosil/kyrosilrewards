...
document.addEventListener('DOMContentLoaded', () => {
    console.log(">>> Kyrosil Bot v161 (Cmd Fix + Breaks) çalıştı! <<<");
    ...
    const translations = { /* ... TÜM TR VE EN METİNLER BURADA ... */
    ...
    async function getGeminiResponse() {
        ...
        try {
            if (!GEMINI_API_KEY || GEMINI_API_KEY === 'SENIN_API_ANAHTARIN_BURAYA') { throw new Error(translations[currentLang].api_fail_error("API Anahtarı ayarlanmamış veya geçersiz.")); }
            ...
            else if (data.promptFeedback && data.promptFeedback.blockReason){ botReply = translations[currentLang].content_blocked(data.promptFeedback.blockReason); console.error("API İçerik Engeli:", data.promptFeedback); }
            ...
            if (thinkingMessageElement && thinkingMessageElement.classList.contains('bot-message') && thinkingMessageElement.textContent.startsWith("...")) { thinkingMessageElement.remove(); }
            addMessage(translations[currentLang].api_fail_error(error.message), 'model'); // Hata mesajını geçmişe ekle
        }
    }
    ...
    function startConversation() {
        ...
        if (currentLang === 'tr') { initialWelcome += "\n(For English, type /lang en)"; }
         ...
         addMessageToChatBoxOnly(initialWelcome, 'bot-message'); // Sadece ekrana yaz, geçmişe değil
         // =============================

        // Sonraki adımı belirle (addBotMessage ile gecikmeli ve geçmişe ekleyerek)
    ...
    // --- Mesaj Gönderme Ana Mantığı (KOMUT ÖNCELİĞİ ve BREAK DÜZELTMESİ) ---
    function sendMessage() {
        ...
        if (messageText !== "") {
            ...
            if (messageText.startsWith('/')) {
                ...
                let commandHandled = true; // Varsayılan olarak komutun işlendiğini varsay
                ...
                switch (command) {
                    ...
                    case 'bilgilerim':
                        let cTk, cMg, cCsi, cSn, cCe, cTg, cTe; // Lokal tanımla
                        currentUserName = loadData(USER_NAME_KEY); currentSocialMedia = loadData(SOCIAL_MEDIA_KEY); currentSocialUser = loadData(SOCIAL_USER_KEY); cTk = loadData(TK_MILES_KEY); cMg = loadData(MAVI_GSM_KEY); cCsi = loadData(CARREFOURSA_INFO_KEY); cSn = loadData(SWISSAIR_NO_KEY); cCe = loadData(CARREFOUR_EU_KEY); cTg = loadData(TIKTAK_GSM_KEY); cTe = loadData(TRENDYOL_EMAIL_KEY);
                        let info = translations[currentLang].my_info_title_basic + "\n"; info += `${translations[currentLang].my_info_name} ${currentUserName || translations[currentLang].not_registered_short}\n`; info += `${translations[currentLang].my_info_platform} ${currentSocialMedia || translations[currentLang].not_registered_short}\n`; info += `${translations[currentLang].my_info_username} ${currentSocialUser || translations[currentLang].not_registered_short}\n`; info += translations[currentLang].my_info_title_sponsor + "\n"; info += `${translations[currentLang].my_info_sponsor_tk} ${cTk || translations[currentLang].not_registered_short}\n`; info += `${translations[currentLang].my_info_sponsor_mavi} ${cMg || translations[currentLang].not_registered_short}\n`; info += `${translations[currentLang].my_info_sponsor_csa} ${cCsi || translations[currentLang].not_registered_short}\n`; info += `${translations[currentLang].my_info_sponsor_swiss} ${cSn || translations[currentLang].not_registered_short}\n`; info += `${translations[currentLang].my_info_sponsor_ceu} ${cCe || translations[currentLang].not_registered_short}\n`; info += `${translations[currentLang].my_info_sponsor_tiktak} ${cTg || translations[currentLang].not_registered_short}\n`; info += `${translations[currentLang].my_info_sponsor_trendyol} ${cTe || translations[currentLang].not_registered_short}`; addBotMessage(info);
                        break; // BREAK EKLENDİ
                    case 'reset': addBotMessage(translations[currentLang].reset_confirm); conversationState = 'awaiting_reset_confirmation'; break; // BREAK EKLENDİ
                    case 'turkishairlines': addBotMessage(translations[currentLang].prompt_tk); conversationState = 'awaiting_tk_no'; break; // BREAK EKLENDİ
                    case 'mavi': addBotMessage(translations[currentLang].prompt_mavi); conversationState = 'awaiting_mavi_gsm'; break; // BREAK EKLENDİ
                    case 'carrefoursa': addBotMessage(translations[currentLang].prompt_carrefoursa); conversationState = 'awaiting_carrefoursa_info'; break; // BREAK EKLENDİ
                    case 'swissair': addBotMessage(translations[currentLang].prompt_swissair); conversationState = 'awaiting_swissair_no'; break; // BREAK EKLENDİ
                    case 'carrefour': addBotMessage(translations[currentLang].prompt_carrefour_eu); conversationState = 'awaiting_carrefour_eu'; break; // BREAK EKLENDİ
                    case 'tiktak': addBotMessage(translations[currentLang].prompt_tiktak); conversationState = 'awaiting_tiktak_gsm'; break; // BREAK EKLENDİ
                    case 'trendyol': addBotMessage(translations[currentLang].prompt_trendyol); conversationState = 'awaiting_trendyol_email'; break; // BREAK EKLENDİ
                    case 'carrefoursaxalgida': addBotMessage(translations[currentLang].prompt_csa_algida); conversationState = 'awaiting_carrefour_no'; break; // BREAK EKLENDİ
                    case 'lang':
                        if (args.length > 0 && (args[0] === 'en' || args[0] === 'tr')) {
                            ...
                            // Dil değişince mevcut soruyu tekrar sor (eğer tanışmadaysak)
                        ...
                        break; // BREAK EKLENDİ
                    default: addBotMessage(translations[currentLang].unknown_command(command)); commandHandled = false; // Bilinmeyen komut işlenmedi sayılır
                }
                // Komut işlendiyse fonksiyonu bitir, aşağıya devam etme
                if (commandHandled) return;
            }
            // === KOMUT KONTROLÜ BİTTİ ===

            // --- State Handling (Onboarding, Reset, Sponsorlar) ---
            // Eğer mesaj komut değilse, o zaman state'e göre işle
            ...
            // Eğer 'idle' durumundaysak ve mesaj komut değilse, selamlaşma veya API çağrısı yap
            else if (conversationState === 'idle') { // Bu kontrol aslında yukarıdaki else'in içinde, tekrar yazmaya gerek yoktu ama netlik için kalsın.
