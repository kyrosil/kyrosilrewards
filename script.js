document.addEventListener('DOMContentLoaded', () => {
    // === DOM Elementleri ===
    // Element seçimlerini DOM yüklendikten sonra yapıyoruz
    ...
    let currentUserName, currentSocialMedia, currentSocialUser; // Bunları fonksiyon içinde yüklüyoruz
    ...
    function addMessage(text, role) {
        ...
        // ChatBox'ın varlığını kontrol edelim
        if (chatBox) {
             chatBox.appendChild(messageElement);
        } else {
            console.error("chatBox elementi bulunamadı!");
            return; // chatBox yoksa ekleme yapma
        }
    ...
    // === Genel Cevap Fonksiyonu ===
     const genericReplies = ["Anladım.", "Hmm, peki.", "İlginç bir nokta.", "Bunu not aldım.", "Tamamdır.", "Devam edebilirsin.", "Peki, başka konu var mı?"];
    ...
    async function getGeminiResponse() {
        ...
        addBotMessage("...", 100); // Thinking indicator ekle (geçmişe değil)
        const thinkingMessageElement = chatBox.querySelector('.bot-message:last-child'); // '...' mesajını bul
        ...
        try {
            ...
            // '...' mesajını güncelle ve cevabı geçmişe ekle
            if (thinkingMessageElement && thinkingMessageElement.textContent === "...") {
                 thinkingMessageElement.querySelector('p').textContent = botReply;
                 addMessage(botReply, 'model'); // Cevabı geçmişe ekle
            } else { addMessage(botReply, 'model'); } // Eğer '...' yoksa direkt ekle
            scrollToBottom();

        } catch (error) {
            console.error("Gemini API isteği başarısız:", error);
             // '...' mesajını kaldırıp hata mesajını gösterelim
            if (thinkingMessageElement && thinkingMessageElement.textContent === "...") { thinkingMessageElement.remove(); }
    ...
    function sendMessage() {
        ...
        if (messageText !== "") {
            ...
            // State handling... (Bu kısım aynı kalıyor)
            if (conversationState !== 'idle') {
                ...
                if (messageText.startsWith('/')) {
                    ...
                    switch (command) {
                         ...
                         case 'bilgilerim':
                             // Verileri burada tekrar yükleyelim ki güncel olsun
                    ...
                    if (commandHandled) return; // Komut işlendiyse API'ye gitme
                }
    ...
    // === Olay Dinleyicileri ===
    // Bunları DOMContentLoaded içine taşıdık
