...
document.addEventListener('DOMContentLoaded', () => {
    console.log(">>> Kyrosil Bot v159 (Lang Hint + TS Fix) çalıştı! <<<");
    ...
    // Mesajı sohbet kutusuna ekleyen ve geçmişe kaydeden fonksiyon (Saat damgası düzeltildi)
    function addMessage(text, role) {
        if (!text || text.trim() === '') return; // Boş mesaj ekleme
        ...
        // Paragraf ve saat damgasını içeren div (CSS ile konumlanacak)
        const contentWrapper = document.createElement('div'); // Bu div önemli
    ...
    // Sadece ekrana mesaj ekleyen fonksiyon (Saat damgası eklendi/düzeltildi)
    ...
    async function getGeminiResponse() {
        ...
        addMessageToChatBoxOnly("...", 'bot-message'); // Thinking indicator (geçmişe eklemez)
        const thinkingMessageElement = chatBox.lastElementChild;
        const payload = { contents: chatHistory };
        try {
            if (!GEMINI_API_KEY || GEMINI_API_KEY === 'AIzaSyDiMIy8gM65-DWVlneXq4oKW4lCqwK0nK4') { throw new Error(translations[currentLang].api_fail_error("API Anahtarı ayarlanmamış veya geçersiz.")); }
            ...
            addMessage(botReply, 'model'); // Cevabı geçmişe ve ekrana ekle
        } catch (error) {
            console.error("Gemini API isteği başarısız:", error);
            if (thinkingMessageElement && thinkingMessageElement.classList.contains('bot-message') && thinkingMessageElement.textContent.startsWith("...")) { thinkingMessageElement.remove(); }
            addMessage(translations[currentLang].api_fail_error(error.message), 'model'); // Hata mesajını geçmişe ve ekrana ekle
        }
    }

    // Konuşmayı başlatan veya devam ettiren fonksiyon (Dil ipucu eklendi)
    function startConversation() {
        console.log("--- startConversation FONKSIYONU ÇALIŞTI! ---");
        chatHistory = [];
        // === İLK KARŞILAMA MESAJI (Dil Seçeneğiyle) ===
        let welcomeMsg = translations[currentLang].welcome_new;
        if (currentLang === 'tr') {
            welcomeMsg += "\n(For English, type /lang en)";
        } else {
            welcomeMsg += "\n(Türkçe için /lang tr yazın)";
        }
        addMessageToChatBoxOnly(welcomeMsg.split("✨")[0] + "✨" + welcomeMsg.split("✨")[1], 'bot-message'); // ✨'den sonrasını da ekle
        // ===============================================
    ...
    // Mesaj Gönderme Ana Mantığı (BUG FIX UYGULANDI + DİL DESTEĞİ)
        ...
        if (messageText !== "") {
            ...
            if (conversationState === 'awaiting_name') {
                ...
                if (allowedSocialMedia.includes(lci)) {
                    currentSocialMedia = lci; saveData(SOCIAL_MEDIA_KEY, currentSocialMedia);
                     // Direkt sonraki adımı sor
            ...
            // --- State Handling Bitti ---
                ...
                if (messageText.startsWith('/')) {
                    ...
                    switch (command) {
                        ...
                        case 'bilgilerim': let cTk,cMg,cCsi,cSn,cCe,cTg,cTe; cUserName=loadData(USER_NAME_KEY);cSocialMedia=loadData(SOCIAL_MEDIA_KEY);cSocialUser=loadData(SOCIAL_USER_KEY);cTk=loadData(TK_MILES_KEY);cMg=loadData(MAVI_GSM_KEY);cCsi=loadData(CARREFOURSA_INFO_KEY);cSn=loadData(SWISSAIR_NO_KEY);cCe=loadData(CARREFOUR_EU_KEY);cTg=loadData(TIKTAK_GSM_KEY);cTe=loadData(TRENDYOL_EMAIL_KEY); let info=translations[currentLang].my_info_title_basic+"\n"; info+=`${translations[currentLang].my_info_name} ${cUserName||translations[currentLang].not_registered_short}\n`; info+=`${translations[currentLang].my_info_platform} ${cSocialMedia||translations[currentLang].not_registered_short}\n`; info+=`${translations[currentLang].my_info_username} ${cSocialUser||translations[currentLang].not_registered_short}\n`; info+=translations[currentLang].my_info_title_sponsor+"\n"; info+=`${translations[currentLang].my_info_sponsor_tk} ${cTk||translations[currentLang].not_registered_short}\n`; info+=`${translations[currentLang].my_info_sponsor_mavi} ${cMg||translations[currentLang].not_registered_short}\n`; info+=`${translations[currentLang].my_info_sponsor_csa} ${cCsi||translations[currentLang].not_registered_short}\n`; info+=`${translations[currentLang].my_info_sponsor_swiss} ${cSn||translations[currentLang].not_registered_short}\n`; info+=`${translations[currentLang].my_info_sponsor_ceu} ${cCe||translations[currentLang].not_registered_short}\n`; info+=`${translations[currentLang].my_info_sponsor_tiktak} ${cTg||translations[currentLang].not_registered_short}\n`; info+=`${translations[currentLang].my_info_sponsor_trendyol} ${cTe||translations[currentLang].not_registered_short}`; addBotMessage(info); break;
                        ...
                        case 'lang':
                            if (args.length > 0 && (args[0] === 'en' || args[0] === 'tr')) { currentLang = args[0]; saveData(LANG_KEY, currentLang); addBotMessage(currentLang === 'en' ? translations.en.lang_set_en : translations.tr.lang_set_tr); }
                            else { addBotMessage(translations[currentLang].lang_fail); } break;
